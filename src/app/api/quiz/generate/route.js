  ...
  try {
    ...
    if (provider !== 'gemini') {
      return NextResponse.json(
        { error: `Provider "${provider}" is not configured. Only Google Gemini is supported.` },
        { status: 400 }
      );
    }
    ...
    const promptSystem = buildPrompt(count, difficulty);
    ...
    if (!process.env.GEMINI_API_KEY) {
      warning =
        'GEMINI_API_KEY is not set, so these questions were assembled from a template rather than generated by AI. Review every question before publishing.';
    } else {
      try {
...
${sanitizedText}`,
        ...
        if (geminiResp && geminiResp.text) {
          ...
          parsedResult = validateQuizOutput(JSON.parse(cleanJson), count);
        }
        if (!parsedResult) {
          warning =
            'The AI response could not be parsed, so these questions were assembled from a template instead. Review every question before publishing.';
        }
      } catch (geminiError) {
        console.warn('Gemini API call failed, falling back:', geminiError?.message || geminiError);
        warning = `The AI provider returned an error (${geminiError?.message || 'unknown error'}), so these questions were assembled from a template instead. Review every question before publishing.`;
      }
    }
    ...
    return NextResponse.json({
      success: true,
      provider: usedFallback ? 'contextual-fallback' : 'gemini',
      usedFallback,
      warning,
      data: parsedResult,
    });
