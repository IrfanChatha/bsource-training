...
export default function TraineeMobileApp({ embedded = false } = {}) {
  ...
  >
                          ...
                          <span>
                            {availableQuizzes[activeTrainingToday.id]
                              ? `Take ${availableQuizzes[activeTrainingToday.id].questions.length}-Q Quiz`
                              : 'Take Quiz'}
                          </span>
                        </button>}
  ...
  >
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400">Knowledge tests</p>
                    </div>
                  </button>
  ...
  >
    ...
    >
                                ...
                                <span>{pastAttempt ? `Score: ${pastAttempt.score}/${pastAttempt.total_questions}` : "Quiz"}</span>
                              </button>}
                          </div>
                        </div>
                      </div>;
  ...
  >
                    <span>
                      {availableQuizzes[selectedTrainingId]
                        ? `Proceed to ${availableQuizzes[selectedTrainingId].questions.length}-Question Evaluation`
    ...
    >
                        {`Submit ${activeQuiz.questions.length}-Q Test`}
                      </button>}
                  </div>
                </div>
  )}
    ...
    /* Earned Certificates List */
              ...
              <div className="space-y-2">
                ...
                {attempts.filter((a) => a.percentage >= 70).length === 0 ? <div className="p-6 text-center rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-400">
                    Complete your first evaluation with 70%+ to unlock your verified certificate.
